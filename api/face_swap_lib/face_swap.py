...
PREDICTOR_PATH = 'models/face_swap/shape_predictor_68_face_landmarks.dat'
...
def face_swap(args):
    args.warp_2d = args.warp_2d == 'true'
    args.correct_color = args.correct_color == 'true'
    # Read images
    src_img = cv2.imread(args.src)
    dst_img = cv2.imread(args.dst)

    # Select src face
    src_points, src_shape, src_face = select_face(src_img, args.src_box)
    # Select dst face
    dst_points, dst_shape, dst_face = select_face(dst_img, args.dst_box)
